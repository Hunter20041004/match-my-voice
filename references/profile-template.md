# Personal voice profile structure
...
- Source notes: sample labels, authorship confidence, speech versus writing. Avoid private paths or account links unless necessary and requested.
- Voice summary: a short description supported by the samples.
- Explicit preferences: rule, scope, and originating feedback.
- Observed patterns: pattern, short de-identified evidence, scope, and uncertainty.
- Core: the habits that hold in every context — what to keep no matter the audience.
  - Core and Context modes are the summary view derived from the scope already recorded on each entry in Explicit preferences and Observed patterns; they hold no independent rules of their own, so record a habit once, where its evidence lives, and let that scope decide which layer it surfaces in.
  - Default: a habit evidenced in only one context belongs to that context's mode, not Core, until a second context supports it, at which point it is promoted to Core — check Context coverage before deciding which layer a habit goes in.
  - Exception: a single-context habit that is clearly context-independent may still enter Core right away, recorded with tentative-inference status, to be confirmed once a second context bears it out.
...
A profile describes expression. It is not a biography, resume, factual knowledge base, or authorization to act on behalf of its owner.
